            ...
            try:
                ...
                if result:
                    ...
                    if new_status != payment.status and new_status:
                        ...
                        logger.info(f"🔄 Статус платежа Platega SBP {transaction_id} обновлен: {payment.status} → {new_status}")
                        ...
                        if new_status == 'confirmed':
                            ...
                            canceled_count += 1
                            logger.debug(f"ℹ️ Статус платежа Platega SBP {transaction_id} изменился: {new_status}")
            ...
            try:
                ...
                if result:
                    ...
                    if new_status != payment.status and new_status:
                        ...
                        logger.info(f"🔄 Статус платежа PlategaCard {transaction_id} обновлен: {payment.status} → {new_status}")
                        ...
                        if new_status == 'confirmed':
                            ...
                            canceled_count += 1
                            logger.debug(f"ℹ️ Статус платежа PlategaCard {transaction_id} изменился: {new_status}")
                            if new_status == 'canceled':
                                user_id = payment.user_id
                                cancel_text = lexicon['payment_cancel']
                                await bot.send_message(user_id, cancel_text, reply_markup=keyboard_payment_cancel())
